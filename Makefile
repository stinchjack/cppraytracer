...
ifeq ($(mode),debug)
	CXXFLAGS=   -DOCTTREE -DDEBUG $(EXPERIMENTFLAGS)  $(shell Magick++-config --cppflags) -Werror -Wall -ggdb -pg -std=c++11
	LDFLAGS =  $(shell Magick++-config --ldflags --libs) -lm -lpng -pg -pthread -lGL -lglfw -lGLEW -lglut

else
   mode = release
	 CXXFLAGS= -DOCTTREE $(EXPERIMENTFLAGS)  $(shell Magick++-config --cppflags) -Werror -fpic -Wall -O3 -std=c++14
